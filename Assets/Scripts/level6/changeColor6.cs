...
public class changeColor6 : MonoBehaviour
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        if(other.gameObject.tag == "Player" && !collided)
        {
            ...
            winLoseController.GetComponent<winLoseControlLevel6>().add_count();
            collided = true;
        }
        else if (other.gameObject.tag == "Player" && collided)
        {
            if (!other.gameObject.GetComponent<InstantMove>().get_iv())
                winLoseController.GetComponent<winLoseControlLevel6>().lose_game();
        }
